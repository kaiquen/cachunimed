...
public class ListFuncionariosController implements Initializable {
    ...
    private void create() throws SQLException {
        ...
        if (!validation()) {
            ...
            if (verifyCpf() && verifyComboBox() && !verifyName_Password(true))
                err = "Tente novamente!";
            ...
            funcionario.create(comboBox.getValue().toString(), textFieldCpfCreate.getText(),
    ...
    private void update() throws SQLException {
        if (tableView.getSelectionModel().isEmpty()) {
            ...
            funcionario.update(Integer.valueOf(labelIdList.getText()), textFieldNameUpdate.getText(),
                    passwordFieldUpdate.getText());
    ...
    private ObservableList<Funcionario> lista() throws SQLException {
        ...
        return FXCollections.observableArrayList(funcionario.selectFuncionarios());
    }
    ...
    private ObservableList<Types> type() throws SQLException {
        ...
        Connection connection = database.connect();
        FuncionarioDAO funcionario = new FuncionarioDAO(connection);
        return FXCollections.observableArrayList(funcionario.selectCargos());
    }
    ...
    private Boolean verifyName_Password(Boolean type) {
        ...
        if (type) {
            verifyName_Password = (!textFieldNameCreate.getText().isEmpty() && !passwordFieldCreate.getText().isEmpty())
                    ? true
                    : false;
        } else {
            verifyName_Password = (!textFieldNameUpdate.getText().isEmpty() && !passwordFieldUpdate.getText().isEmpty())
                    ? true
                    : false;
        }
    ...
    private Boolean verifyCpf() {
        boolean verifyCPF = (!textFieldCpfCreate.getText().isEmpty() && textFieldCpfCreate.getText().length() == 11
                && textFieldCpfCreate.getText().matches("[0-9]+")) ? true : false;
        return verifyCPF;
    }
